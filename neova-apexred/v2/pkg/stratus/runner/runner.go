...
func (m *runnerImpl) Detonate() error {
	startTime := time.Now()
	log.Println("[START] Detonating attack technique:", m.Technique.ID, "at", startTime.Format(time.RFC3339))
	willWarmUp := true
	var err error
	...
	log.Println("[END] Finished detonating attack technique:", m.Technique.ID, "at", endTime.Format(time.RFC3339), "(Duration:", duration, ")")
	return nil
}
